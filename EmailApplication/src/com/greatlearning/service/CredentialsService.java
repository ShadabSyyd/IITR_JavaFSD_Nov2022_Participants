...
public class CredentialsService implements ICredentials {
	...
	public String generatePassword() {
		...
		for (int i=0; i < 8; i++) {
			...
			int boundValue = allValues.length();//length of the allValues string 
			int randomIndex = randomObj.nextInt(boundValue);//returns random number between 0 and boundValue	
			char randomChar = allValues.charAt(randomIndex);//get the character (in allValues string) which is at randomIndex	
